...
def get_cookie_value(cookies, name: str) -> str:
    ...
    for domain in (".google.com", ".googleusercontent.com", None):
        try:
            value = (
                cookies.get(name, domain=domain)
                if domain is not None
                else cookies.get(name)
            )
        except TypeError:
...
AUTO_DELETE_CHAT = (
    os.environ.get("AUTO_DELETE_CHAT", "true").lower() == "true" and not TEMPORARY_CHAT
)
...
async def fetch_readable_chat_response(
    client: GeminiClient, cid: str, retry_delays: List[int]
) -> Optional[object]:
...
async def background_verify_chat_persistence(
    client: GeminiClient, cid: str, source: str
):
...
async def validate_gemini_client_session(client: GeminiClient, source: str):
    ...
    try:
        response = await client.generate_content(
            SESSION_VALIDATION_PROMPT, temporary=False
        )
        ...
        recovered = await fetch_readable_chat_response(
            client, validation_cid, [1, 3, 8]
        )
        if not recovered or response_indicates_auth_failure(
            getattr(recovered, "text", "") or ""
        ):
            raise ValueError(
                "validation probe chat was not readable from Gemini history"
            )
        ...
        if validation_cid:
            try:
                ...
                logger.debug(
                    "Failed to delete Gemini validation chat %s", validation_cid
                )
...
def load_or_generate_secret() -> str:
    ...
    if os.path.exists(SECRET_FILE_PATH):
        try:
            ...
            logger.warning(
                f"Failed to read secret file, trying to generate a new one: {e}"
            )
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Failed to set restrictive permissions on {SECRET_FILE_PATH}: {e}"
            )
...
def remove_gemini_watermark(image_bytes: bytes) -> bytes:
    ...
    try:
        with Image.open(io.BytesIO(image_bytes)) as img:
            ...
            if x < 0 or y < 0:
                logger.warning(
                    f"Image too small for watermark removal: {width}x{height}"
                )
                return image_bytes
...
if not SECURE_1PSID or not SECURE_1PSIDTS:
    logger.warning(
        "Gemini credentials are missing; set SECURE_1PSID and SECURE_1PSIDTS before serving requests."
    )
else:
    logger.info(
    ...
    async with gemini_client_lock:
        ...
        try:
            ...
            for source, psidts in attempts:
                ...
                try:
                    logger.info(
                        "Initializing Gemini client using %s credentials", source
                    )
                    ...
                    last_error = e
                    logger.warning(
                        f"Gemini session setup failed using {source} 1PSIDTS: {e}"
                    )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to initialize Gemini client: {str(e)}"
            )
    return gemini_client
    ...
    if hasattr(response, "images") and response.images:
        for img in response.images:
            ...
            if img_url:
                ...
                proxy_url = (
                    f"{base_url}/gemini-proxy/image?url={quote(img_url)}&sig={sig}"
                )
        ...
        if request.stream:
            ...
            async def generate_stream():
                try:
                    ...
                    async for chunk in gemini_client.generate_content_stream(
                        ...
                        if (
                            AUTO_DELETE_CHAT
                            and captured_cid is None
                            and hasattr(chunk, "metadata")
                            and chunk.metadata
                            and len(chunk.metadata) > 0
                        ):
                        ...
                        if (
                            ENABLE_THINKING
                            and hasattr(chunk, "thoughts_delta")
                            and chunk.thoughts_delta
                        ):
                        ...
                        if hasattr(chunk, "text_delta") and chunk.text_delta:
                            ...
                            if safe_to_yield:
                                yield make_chunk(
                                    {"content": postprocess_text(text_buffer)}
                                )
                                text_buffer = ""

                        # Handle inline images as they arrive
                        if (
                            hasattr(chunk, "images")
                            and chunk.images
                            and len(chunk.images) > yielded_images
                        ):
                    ...
                    if (
                        last_metadata
                        and len(last_metadata) > 0
                        and not AUTO_DELETE_CHAT
                    ):
                        asyncio.create_task(
                            background_verify_chat_persistence(
                                gemini_client, last_metadata[0], "stream"
                            )
                        )
                except Exception as e:
                    ...
                    if AUTO_DELETE_CHAT and captured_cid:
                        asyncio.create_task(
                            background_delete_chat(gemini_client, captured_cid)
                        )
            ...
            try:
                ...
                if (
                    ...
                    asyncio.create_task(
                        background_verify_chat_persistence(
                            gemini_client, response.metadata[0], "non-stream"
                        )
                    )
                elif not getattr(response, "metadata", None):
                    logger.warning(
                        "Non-stream response returned no Gemini metadata. This request may not map to a persistent Gemini chat."
                    )
...
async def proxy_image(url: str, sig: str):
    ...
    async with httpx.AsyncClient(
        http2=True, cookies=jar, follow_redirects=True
    ) as client:
        try:
            ...
            async with client.stream(
                "GET", fetch_url, timeout=15.0, headers=headers
            ) as resp:
                ...
                async for chunk in resp.aiter_bytes():
                    ...
                    if len(content) > MAX_BYTES:
                        logger.warning(
                            f"Image too large: {url} (exceeded {MAX_BYTES} bytes)"
                        )
                ...
                if not upstream_content_type.startswith("image/"):
                    logger.warning(
                        f"Rejected non-image Content-Type: {upstream_content_type} for {url}"
                    )
